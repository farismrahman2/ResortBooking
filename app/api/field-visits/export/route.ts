...
const HEADERS = [
  ...
  'contacts_count', 'decision_signoff (CON.04)', 'best_time_to_call (CON.05)', 'preferred_channel (CON.06)',
  ...
  'rooms_needed (REQ.07)', 'annual_event_spend (REQ.08)', 'peak_months (REQ.09)', 'transport (REQ.10)',
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const csv = toCsv(rows.map((r) => ({
      ...
      'best_time_to_call (CON.05)':   r.best_time_to_call ?? '',
      'preferred_channel (CON.06)':   (r.preferred_channel ?? []).join(' | '),
      ...
      'transport (REQ.10)':           (r.transport ?? []).join(' | '),

...
30. **Start a checkout without a charge** (`9e494fc`): new `<StartCheckoutButton>` (`components/checkout/StartCheckoutButton.tsx`) sits in the right rail's empty-state slot — calls `getOrCreateDraftCheckout` (now `revalidatePath`'s `/checkout/<id>` and `/bookings/<id>`) then `router.refresh()`. Once the draft exists, the gated UI (Payments, Discount, GuestCount, Finalize) renders automatically. Removes the prior workaround of inserting a fake charge to unlock those tools.
31. **Front desk: scope cap on the checkout view** (`84ed8e6`, `e5d5d74`): front_desk users now see only bookings whose `visit_date` falls within `[today − 3d, today + 2d]`. `listCheckoutCandidates` gained `maxVisitDate` + `minVisitDate` params; `app/(agent)/checkout/page.tsx` reads `getCurrentUserContext()` and passes the window for `front_desk`. The detail page (`app/(agent)/checkout/[bookingId]/page.tsx`) mirrors the check and `redirect`s to `/403?from=checkout` if a front_desk user URL-types into a booking outside the window. Other roles (admin, manager, accountant) keep the default 30-day past window with no future cap.
...
- **Front desk's scope window is enforced in two places, not one.** The list page filters via `listCheckoutCandidates({ minVisitDate, maxVisitDate })`; the detail page does its own role + `visit_date` check after fetching the booking. If you change the window (currently `today ± 3/2 days`), update both call sites — there's no shared helper yet because the rule is small and stable.

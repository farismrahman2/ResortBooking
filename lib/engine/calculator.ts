...
/**
 * Format using LOCAL date parts, not toISOString(). These Dates are built as
 * local midnight (`new Date(ymd + 'T00:00:00')`); toISOString() converts to
 * UTC, which in Asia/Dhaka (UTC+6) lands on the PREVIOUS day — so holiday
 * lookups missed every holiday by one day. getDay() in isFriday() is already
 * local, so this keeps both checks on the same calendar.
 */
...
export function calculateDaylong(inputs: DaylongInputs): CalculationResult {
  ...
  // Room charges — skip complimentary rooms (unit_price === 0)
